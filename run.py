...
def get_sales_data():
    """
    Get sales figures input from the the user
    """
    while True:
...
def validate_data(values):
    ...
    Raises valueError if strings can not be converted to ints,
    ...
    try:
        ...
        if len(values) != 6:
            raise ValueError(
                f"Exactly 6 values are required, you entered {len(values)} values"
            )
...
def calculate_surplus_data(sales_row):
    ...
    stock_row = stock[-1]
    stock_row = [int(d) for d in stock_row]
    surplus_data = [stock - sales for stock, sales in zip(stock_row, sales_row)]
    return surplus_data
